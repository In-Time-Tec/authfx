  ...
  return h.li(
    ...
    [
      button(
        {
          variant: "ghost",
          onClick: isLocked ? undefined : ToggledSidebarGroup({ group: group.title }),
          isDisabled: isLocked,
          class: cn(
          ...
          attributes: [h.DataAttribute("sidebar-group", group.title), h.AriaExpanded(isOpen), h.AriaControls(panelId)],
        },
        [
          h.span([], [group.title]),
...
const breadcrumbs = (title: string): Html =>
  breadcrumb({}, [
    breadcrumbList({}, [
      breadcrumbItem({}, [breadcrumbLink({ href: "/" }, ["Authfx"])]),
      breadcrumbSeparator({}),
